...
function renderEvidencias() {
  ...
  if (items.length === 0) { 
    grid.innerHTML = emptyState('📸', 'Sin evidencias registradas', 'Sé el primero en registrar una incidencia'); 
    return; 
  }
  ...
  grid.innerHTML = items.map(e => {
    ...
    const isRevisado = e.estado === 'revisado';
    const isPendiente = e.estado === 'pendiente';

    // Build incident details
    let detailsHtml = '';
    if (e.numeroPedido || e.numeroRecogida) {
      detailsHtml += `<div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:10px;padding:10px;background:#f8f9fa;border-radius:8px;">`;
      if (e.numeroPedido) detailsHtml += `<span style="font-size:12px;color:var(--secondary);font-weight:500;">🛒 Pedido: <strong>#${e.numeroPedido}</strong></span>`;
      if (e.numeroRecogida) detailsHtml += `<span style="font-size:12px;color:var(--secondary);font-weight:500;">📋 Recogida: <strong>#${e.numeroRecogida}</strong></span>`;
      detailsHtml += `</div>`;
    }
    ...
    let commentHtml = '';
    if (e.comentario) {
      commentHtml = `<div style="margin-top:12px;padding:12px;background:#FFF8E1;border-radius:8px;border-left:3px solid var(--warning);">
        <div style="font-size:11px;font-weight:600;color:#E65100;margin-bottom:4px;">💬 Comentario</div>
        <div style="font-size:13px;color:var(--text);line-height:1.5;">${e.comentario}</div>
      </div>`;
    }
    ...
    if (isOwner) {
      if (isCerrado) {
        closeBtnHtml = `<button class="btn btn-info" onclick="toggleIncidenteCerrado('${e.id}', false)">🔓 Reabrir incidente</button>`;
      } else if (isRevisado) {
        closeBtnHtml = `<button class="btn btn-secondary" onclick="toggleIncidenteCerrado('${e.id}', true)">🔒 Cerrar incidente</button>`;
      }
    }

    return `<div class="card ${!isOwner ? 'evidence-shared' : ''}" style="${isCerrado ? 'opacity:0.7;' : ''}">
      <div class="card-header">
        <div class="card-icon" style="background:${isOwner ? '#FFF3E0' : '#E3F2FD'};">📸</div>
        <span class="card-status ${getStatusClass(e.estado)}">${getStatusIcon(e.estado)} ${e.estado}</span>
      </div>
      <div class="card-title">${e.tipo}${isCerrado ? ' <span style="font-size:11px;color:#999;">(Cerrado)</span>' : ''}</div>
      ...
      ${e.imagenes && Array.isArray(e.imagenes) && e.imagenes.length > 0 ? `<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;">${e.imagenes.map(img => `<img src="${img}" style="width:60px;height:60px;object-fit:cover;border-radius:8px;cursor:pointer;" onclick="window.open('${img}')">`).join('')}</div>` : ''}
      ${hasVideo ? `<div style="margin-top:16px;padding:12px;background:#E8F5E9;border-radius:var(--radius-sm);"><div style="font-size:13px;font-weight:600;color:#2E7D32;margin-bottom:8px;">🎥 Video de respuesta disponible</div><video class="video-preview" controls><source src="${e.videoUrl}" type="video/mp4"></video><a href="${e.videoUrl}" download class="btn download-btn" style="width:100%;margin-top:10px;justify-content:center;">⬇️ Descargar video</a></div>` : isOwner && !isCerrado ? `<div style="margin-top:12px;padding:10px;background:#FFF3E0;border-radius:var(--radius-sm);font-size:12px;color:#E65100;">⏳ Esperando video de respuesta del administrador...</div>` : ''}
      <div class="card-meta" style="margin-top:8px;">
        <span style="font-size:11px;color:#999;">Registrado: ${formatFirestoreDate(e.creado)}</span>
      </div>
      ${isOwner ? `<div class="card-actions">
        ${!isCerrado ? `<button class="btn btn-secondary" onclick="editItem('evidencias','${e.id}')">✏️ Editar</button>` : ''}
        ${closeBtnHtml}
        <button class="btn btn-danger" onclick="deleteItem('evidencias','${e.id}')">🗑️ Eliminar</button>
      </div>` : ''}
    </div>`;
  }).join('');
}
