...
function loadData() {
    try {
        ...
        const parsed = data ? JSON.parse(data) : { expenses: [], incomes: [], budgets: [], automaticIncomes: [], automaticExpenses: [], monthlyAutomaticExpenses: [] };
        // Ensure arrays exist
        return {
            expenses: Array.isArray(parsed.expenses) ? parsed.expenses : [],
            incomes: Array.isArray(parsed.incomes) ? parsed.incomes : [],
            budgets: Array.isArray(parsed.budgets) ? parsed.budgets : [],
            automaticIncomes: Array.isArray(parsed.automaticIncomes) ? parsed.automaticIncomes : [],
            automaticExpenses: Array.isArray(parsed.automaticExpenses) ? parsed.automaticExpenses : [],
            monthlyAutomaticExpenses: Array.isArray(parsed.monthlyAutomaticExpenses) ? parsed.monthlyAutomaticExpenses : []
        };
    } catch (e) {
...
// Display expenses
function displayExpenses(search = '', filter = '') {
    const data = loadData();
...
// Display incomes
function displayIncomes(search = '', filter = '') {
    const data = loadData();
