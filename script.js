...
const GENERAL_PIN_HASH = '0ea92cf4fa0f2f0f6c6f2d404f478ce1f37f08f5de330f8e9417df1914f10898';
const DOC_DOWNLOAD_PIN_HASH = '453e406dcee4d18174d4ff623f52dcd3969d6ef9f05078b4842e7d271cfbda3f';
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    window.verifyPin = async function() {
        ...
        const expectedPinHash = pinContext === 'doc-download' ? DOC_DOWNLOAD_PIN_HASH : GENERAL_PIN_HASH;
        ...
        const ingresosSmall = document.querySelector('.metric-ingresos small');
        const gastosMesSmall = document.querySelector('.metric-gastosmes small');

        if (pinContext === 'general') {
            if (saldoSmall) {
            ...
            if (ingresosSmall) {
                ingresosSmall.textContent = formatEUR(totalIncomeMonth);
                ingresosSmall.parentElement.classList.remove('censored');
                ingresosSmall.parentElement.onclick = null;
            }
            if (gastosMesSmall) {
                gastosMesSmall.textContent = formatEUR(totalExpenseMonth);
                gastosMesSmall.parentElement.classList.remove('censored');
                gastosMesSmall.parentElement.onclick = null;
            }
            setPinMessage('success', 'Métricas desbloqueadas correctamente.');
        }
    ...
    function runProtectedAction(action, context = 'general') {
    ...
    document.querySelectorAll('.doc-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            }, action === 'download' ? 'doc-download' : 'general');
        });
    });
    ...
    function enforceDashboardCensoredState() {
        const secureMetrics = [
            '.metric-ingresos',
            '.metric-gastosmes',
            '.metric-saldo',
        ...
        secureMetrics.forEach(selector => {
            const metric = document.querySelector(selector);
            const small = metric?.querySelector('small');
            if (!metric || !small) return;
            if (!metric.classList.contains('censored')) return;
            small.textContent = '*****';
            metric.onclick = () => checkPassword(metric.classList.contains('metric-gastos') ? 'gastos' : 'saldo');
        });
    }
    ...
    function refreshDashboardFromFinance() {
        ...
        const ingresosMetric = document.querySelector('.metric-ingresos');
        const gastosMesMetric = document.querySelector('.metric-gastosmes');

        if (dashIncomeMonth) {
            dashIncomeMonth.textContent = ingresosMetric?.classList.contains('censored')
                ? '*****'
                : formatEUR(totalIncomeMonth);
        }
        if (dashExpenseMonth) {
            dashExpenseMonth.textContent = gastosMesMetric?.classList.contains('censored')
                ? '*****'
                : formatEUR(totalExpenseMonth);
        }
